...
open class HiJsonLoadHelper:NSObject {
    ...
    @objc public static func fetchJson(named name: String,moduled module:String) ->Dictionary<String, Any> {
        ...
        do {
            if let data = data {
                jsonDictonary = try JSONSerialization.jsonObject(with: data as Data, options: .mutableContainers) as? [String : Any]
            }
        } catch {
        }
        if error != nil {
        ...
        return jsonDictonary as! Dictionary<String, Any>;
    }
}
